...
from ticket t left join otrs.vw_campos_extras cex on cex.tickect_id  = t.id 
 WHERE YEAR((Select max(kpi.finish_time) from vw_kpi_rate as kpi where t.id=kpi.ticket_id)) =  $Ano_criado and
                MONTH((Select max(kpi.finish_time) from vw_kpi_rate as kpi where t.id=kpi.ticket_id)) = $Mes_criado
