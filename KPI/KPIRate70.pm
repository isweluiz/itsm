...
package Kernel::System::Stats::Static::KPIRate70;
...
sub Run {
    ...
    $DBObject->Prepare(
        SQL => "
              select 
                count(*) as total,
                round((sum(case when rate_solution < 70 then 1 else 0 end)*100)/count(*),2) as rate_0_70,
                round((sum(case when rate_solution >= 70 and rate_solution < 90 then 1 else 0 end)*100)/count(*),2) as rate_70_90,
