...
create or replace view vw_aux_client_tkt as
select distinct cuc.customer_id as user_id,
t4.id id_tkt
from customer_user cuc
inner join ticket t4 on t4.customer_user_id = cuc.customer_id
group by t4.id

create or replace view vw_aux_finaliza_tkt as
select max(vkt.user_finish) as analista_solucionador,
vkt.ticket_id from vw_kpi_tickets vkt where user_finish is not null
group by ticket_id
...
t2.title as assunto ,
q2.name as fila,
t2.customer_user_id as cliente,
sr.send_to enviado_para,
usrfn.analista_solucionador,
sqt.id id_question,
sqt.question as pergunta,
